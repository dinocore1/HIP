...
For the purposes of providing reliable Proof of Coverage maps and for determining Hotspot density, it is desirable for Helium Hotspots to remain in a single location and rarely move. Traditionally, the Hotspot owners are required to provide the location via an Assert Location Transaction. Anytime a Hotspot changes location, the owner is suppose to create another Assert Location and pay the associated fee. This is necessary because traditional Hotspot hardware designs have no requirement for GPS receivers and so the Assert Location mechanism is the only way to provide Hotspot locations to the blockchain. Secure Concentrators differ from traditional Hotspot designs because they provide a trusted GPS location data. This presents a problem; which location should the blockchain Oracles use? We propose using the GPS location provided by the Secure Concentrator as that will always be more accurate and up-to-date than the Assert Location. We propose the following: a Secure Hotspot location is determined by computing the centroid of the past 30 days of GPS metadata provided from received packets. Then we compute the standard deviation distance between centroid and all GPS locations reported from the past 30 days. Secure Hotspots are only eligible for receiving PoC rewards if the resulting standard deviation is less than 20 meters. Secure Hotspot owners can also create an Assert Location transaction when moving Hotspot so they will not have to wait the 30 day cool down period before continuing to earn PoC at the new location.
