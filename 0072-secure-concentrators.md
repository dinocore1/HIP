...
## Secure Hotspot Location
For the purposes of providing reliable Proof of Coverage maps and for determining Hotspot density, it is desirable for Helium Hotspots to remain in a single location and rarely move. Traditionally, the Hotspot owners are required to provide the location via an Assert Location Transaction. Anytime a Hotspot changes location, the owner is suppose to create another Assert Location and pay the associated fee. This is necessary because traditional Hotspot hardware designs have no requirement for GPS receivers and so the Assert Location mechanism is the only way to provide Hotspot locations to the blockchain. Secure Concentrators differ from traditional Hotspot designs because they provide a trusted GPS location data. Now that the we have two sources for providing Hotspot location, which should the blockchain Oracles use? Because asserting location is a manual process and has a monetary cost, it is reasonable to assume that there are some Hotspot locations that are not as accurate or updated as they could be. This presents a problem for providing accurate Proof of Coverage maps and density calculations. We propose using the GPS location provided by the Secure Concentrator as that will always be more accurate and up-to-date than the Assert Location.

We propose the PoC Oracle shall determine the location of a Secure Hotspot using the weighted GPS average (centroid) from the trailing 30 day period. Every day, the PoC Oracle will update the location using the GPS coordinates reported from a latest PoC witness receipt using a weighting average of 1. If there is no witnessing during that period, the previous GPS coordinates shall be used. If there is a Location Assert transaction for that Hotspot within the period, the asserted location shall be used in the computed average with a weight of 30. Secure Hotspots are only eligible for receiving PoC rewards if the weighted standard standard deviation distance from the centroid to all data points is less than 20 meters.
